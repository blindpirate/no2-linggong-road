...
CursorWidget::CursorWidget(QWidget *parent) :
    QWidget(parent),
    flashTimerID(startTimer(CURSORANIMATIONINTERVAL)),
    moveTimerID(0),
    cursorIsBig(true),
    currentPointX(WINDOWWIDTH/BASEWIDTH/2*BASEWIDTH),
    currentPointY(WINDOWHEIGHT/BASEHEIGHT/2*BASEHEIGHT),
    moveEndX(0),
    moveEndY(0)
{
    this->setAttribute(Qt::WA_DeleteOnClose);
    this->setGeometry(WINDOWWIDTH/BASEWIDTH/2*BASEWIDTH,WINDOWWIDTH/BASEHEIGHT/2*BASEWIDTH,BASEWIDTH,BASEHEIGHT);
    cursor.load(QApplication::applicationDirPath()+QString("/data/image/other/cursor.png"));
    if(cursor.isNull())quitApp(ERRORIMAGEFILENOTFOUND);
}
...
void CursorWidget::timerEvent(QTimerEvent *event)
{
    ...
    {
        int dis=sqrt(static_cast<double>(((currentPointX-moveEndX)*(currentPointX-moveEndX)+(currentPointY-moveEndY)*(currentPointY-moveEndY))));
        ...
        {
            currentPointX+=
                    (currentPointX>moveEndX?-1:1)
                    *abs(currentPointX-moveEndX)
                    *CURSORMOVEPIXEL
                    /dis;
            currentPointY+=
                    (currentPointY>moveEndY?-1:1)
                    *abs(currentPointY-moveEndY)
                    *CURSORMOVEPIXEL
                    /dis;
