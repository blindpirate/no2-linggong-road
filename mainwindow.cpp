...
MainWindow::MainWindow(QWidget *parent) :
    ...
    story(NULL),key(new KeyboardControl),
    startButton(NULL),aboutButton(NULL),quitButton(NULL),loadButton(NULL),
    logoHeight(0),
    currentSelectButton(0),
    userControlOn(false),
    background(QPixmap(QApplication::applicationDirPath()+QString("/data/image/other/background.png"))),
    neverawayLogo(QPixmap(QApplication::applicationDirPath()+QString("/data/image/other/neveraway.png"))),
    outlight(QPixmap(QApplication::applicationDirPath()+QString("/data/image/other/outlight.png")))
{
    ui->setupUi(this);

    DBIO dbio;
    dbio.getKeySettings(key->upKey,key->downKey,key->leftKey,
                        key->rightKey,key->actionKey,key->cancelKey,key->menuKey);
...
//    connect(start->quitButton,SIGNAL(clicked()),this,SLOT(onQuitGame()));
    ...
    QPropertyAnimation *animation=new QPropertyAnimation(this,"logoHeight",this);
...
{//这个函数主要是为了截留鼠标移动的消息,使得可以让鼠标对应的按钮发光
