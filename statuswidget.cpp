...
StatusWidget::StatusWidget(QWidget *parent,
    ...
    infoWidget(new RoleInfoWidget(this,roleInfo)),
    currentRoleID(1),
...
void StatusWidget::paintEvent(QPaintEvent *)
{
    ...
    painter.drawText(10,430,200,40,Qt::AlignVCenter|Qt::AlignLeft,QString("金钱 %1").arg(carriedCash));
    ...
    else
    {
        ...
        painter.drawRoundedRect(160+static_cast<int>(currentTabIndex)*60,20,60,20,5,5);
    }
    ...
    case StatusWeapon:
    {
        ...
        for(int i=0;i<MAXWEAPONCOUNT;++i)
        {
            ...
            if(!currentPaintWeaponID)
                break;
            painter.drawText(230,90+i*30,140,30,Qt::AlignCenter,(*pArticleIDToInfoMap)[currentPaintWeaponID].name);
            painter.drawText(350,90+i*30,40,30,Qt::AlignCenter,QString("%1").arg((*pArticleIDToInfoMap)[currentPaintWeaponID].attack));
            painter.drawText(390,90+i*30,40,30,Qt::AlignCenter,QString("%1").arg((*pArticleIDToInfoMap)[currentPaintWeaponID].accurcy));
        }
        //画武器框结束
        ...
        for(int i=0;i<4;++i,++iter)
        {
            ...
            painter.drawText(230,245+i*30,110,30,Qt::AlignCenter,(*pArticleIDToInfoMap)[currentPaintWeaponID].name);
            painter.drawText(340,245+i*30,40,30,Qt::AlignCenter,QString("%1").arg((*pArticleIDToInfoMap)[currentPaintWeaponID].attack));
            painter.drawText(380,245+i*30,40,30,Qt::AlignCenter,QString("%1").arg((*pArticleIDToInfoMap)[currentPaintWeaponID].accurcy));
            painter.drawText(420,245+i*30,30,30,Qt::AlignCenter,QString("%1").arg(pCarriedArticleSet->count(currentPaintWeaponID)));

        }
        //画可用武器区结束
        painter.setPen(QColor(242,190,70));
        if(currentSelectedItemID)
            painter.drawText(225,365,230,90,Qt::TextWordWrap,(*pArticleIDToInfoMap)[currentSelectedItemID].description);

        //画武器描述区结束
        painter.setPen(Qt::white);
        painter.setBrush(Qt::NoBrush);
        if(focus==StatusFocusOnEquipment||
                focus==StatusFocusOnItemSelected)
        {
            ...
            else
            {
                ...
                painter.drawRoundedRect(230,220+(currentItemIndex-currentEquipCount)*30,220,20,2,2);

                painter.setPen(QPen(Qt::white,1));
                painter.setBrush(QBrush(Qt::white));
                rightArrow[0]=QPoint(448,230+(currentItemIndex-currentEquipCount)*30);
                rightArrow[1]=QPoint(443,235+(currentItemIndex-currentEquipCount)*30);
                rightArrow[2]=QPoint(443,225+(currentItemIndex-currentEquipCount)*30);
                painter.drawPolygon(rightArrow,3);
            }
            //画选中框
        }
        if(focus==StatusFocusOnItemSelected)
        {
            ...
            {
                painter.drawRoundedRect(380,105+(currentItemIndex-1)*30,80,30,4,4);
                if(currentItemSelectedCount==0)
                    painter.setPen(Qt::gray);
                else
                    painter.setPen(Qt::red);
                painter.drawText(380,105+(currentItemIndex-1)*30,80,30,Qt::AlignCenter,"卸下");
            }
            else
            {
                painter.drawRoundedRect(380,230+(currentItemIndex-currentEquipCount)*30,80,30,2,2);
                if(currentItemSelectedCount==0)
                    painter.setPen(Qt::gray);
                else
                    painter.setPen(Qt::red);
                painter.drawText(380,230+(currentItemIndex-currentEquipCount)*30,80,30,Qt::AlignCenter,"装备");
            }
        }//画"装备/卸下菜单"
    }
        break;
    case StatusShield:
    {
        ...
        if(currentPaintShieldID)
        {
            ...
            painter.drawText(230,90,160,30,Qt::AlignCenter,(*pArticleIDToInfoMap)[currentPaintShieldID].name);
            painter.drawText(390,90,60,30,Qt::AlignCenter,QString("%1").arg((*pArticleIDToInfoMap)[currentPaintShieldID].attack));
        }
        //画武器框结束
        ...
        for(int i=0;i<4;++i,++iter)
        {
            ...
            painter.drawText(230,245+i*30,140,30,Qt::AlignCenter,(*pArticleIDToInfoMap)[currentPaintShieldID].name);
            painter.drawText(370,245+i*30,40,30,Qt::AlignCenter,QString("%1").arg((*pArticleIDToInfoMap)[currentPaintShieldID].attack));
            painter.drawText(410,245+i*30,40,30,Qt::AlignCenter,QString("%1").arg(pArticleIDToInfoMap->count(currentPaintShieldID)));
                ...
                focus==StatusFocusOnItemSelected)
        {
            ...
            else
            {
                ...
                painter.setPen(QPen(Qt::white,1));
                painter.setBrush(QBrush(Qt::white));
                rightArrow[0]=QPoint(445,230+(currentItemIndex-currentEquipCount)*30);
                rightArrow[1]=QPoint(440,225+(currentItemIndex-currentEquipCount)*30);
                rightArrow[2]=QPoint(440,235+(currentItemIndex-currentEquipCount)*30);
                painter.drawPolygon(rightArrow,3);
            }
    ...
    case StatusMedicine:
    {
        ...
        for(int i=0;i<MAXMEDICINECOUNT;++i)
        {
            ...
            if(!currentPaintMedicineID)
                break;
            painter.drawText(230,90+i*30,220,30,Qt::AlignCenter,(*pArticleIDToInfoMap)[currentPaintMedicineID].name);
        }
        //画武器框结束
        ...
        for(int i=0;i<MAXMEDICINECOUNT;++i,++iter)
        {
            ...
            painter.drawText(230,245+i*30,160,30,Qt::AlignCenter,(*pArticleIDToInfoMap)[currentPaintMedicineID].name);
            painter.drawText(390,245+i*30,60,30,Qt::AlignCenter,QString("%1").arg(pCarriedArticleSet->count(currentPaintMedicineID)));
                ...
                focus==StatusFocusOnItemSelected)
        {
            ...
            if(currentItemIndex<currentEquipCount+1)//在上面的框里
            {
                painter.drawRoundedRect(230,95+(currentItemIndex-1)*30,220,20,2,2);
        ...
        if(focus==StatusFocusOnItemSelected)
        {
            ...
            if(currentItemIndex<currentEquipCount+1)
            {
                painter.drawRoundedRect(380,105+(currentItemIndex-1)*30,80,60,4,4);
                if(currentItemSelectedIndex==0&&currentItemSelectedCount==2)
                {//两个菜单条目,都处于可选状态
                    painter.setPen(Qt::red);
                    painter.drawText(380,105+(currentItemIndex-1)*30,80,30,Qt::AlignCenter,"卸下");
                    painter.setPen(Qt::white);
                    painter.drawText(380,135+(currentItemIndex-1)*30,80,30,Qt::AlignCenter,"使用");
                }
                else if(currentItemSelectedIndex==0&&currentItemSelectedCount==1)
                {//两个菜单条目,只有一个可选,另一个是灰色
                    painter.setPen(Qt::red);
                    painter.drawText(380,105+(currentItemIndex-1)*30,80,30,Qt::AlignCenter,"卸下");
                    painter.setPen(Qt::gray);
                    painter.drawText(380,135+(currentItemIndex-1)*30,80,30,Qt::AlignCenter,"使用");
            ...
            {
                painter.drawRoundedRect(380,230+(currentItemIndex-currentEquipCount)*30,80,60,2,2);
                if(currentItemSelectedCount==0)
                {
                    painter.setPen(Qt::gray);
                    painter.drawText(380,230+(currentItemIndex-currentEquipCount)*30,80,30,Qt::AlignCenter,"装备");
                    painter.setPen(Qt::gray);
                    painter.drawText(380,260+(currentItemIndex-currentEquipCount)*30,80,30,Qt::AlignCenter,"使用");
                }
                else if(currentItemSelectedCount==2)
                {//两个菜单条目,都处于可选状态
                ...
                else if(currentEquipCount==MAXMEDICINECOUNT)
                {//两个菜单条目,只有一个可选,另一个是灰色,这时候的itemIndex肯定是0,如果装备区是满的,证明现在亮的应该是使用
                    painter.setPen(Qt::gray);
                    painter.drawText(380,230+(currentItemIndex-currentEquipCount)*30,80,30,Qt::AlignCenter,"装备");
                    painter.setPen(Qt::red);
                    painter.drawText(380,260+(currentItemIndex-currentEquipCount)*30,80,30,Qt::AlignCenter,"使用");
    ...
    case StatusArticle:
    {
        ...
        {
            painter.drawText(230,90,220,30,Qt::AlignCenter,(*pArticleIDToInfoMap)[currentPaintArticleID].name);
        }
        //画武器框结束
        ...
        for(int i=0;i<4;++i,++iter)
        {
            ...
            painter.drawText(230,245+i*30,160,30,Qt::AlignCenter,(*pArticleIDToInfoMap)[currentPaintArticleID].name);
            painter.drawText(390,245+i*30,60,30,Qt::AlignCenter,QString("%1").arg(pArticleIDToInfoMap->count(currentPaintArticleID)));
...
{//如果装备区满了不允许进行装备操作
    ...
    case StatusFocusOnEquipment:
        ...
        case StatusWeapon:
            ...
            if((currentItemIndex>currentEquipCount&&currentEquipCount==MAXWEAPONCOUNT)
                    ||(currentItemIndex<currentEquipCount+1&&currentEquipCount==1))
            {//在下面的框里并且武器区已满,或者只剩了一个武器
        ...
        case StatusArticle:
            if(currentItemIndex>currentEquipCount&&currentEquipCount==MAXSHIELDCOUNT)
            {//在下面的框里并且武器区已满
                currentItemSelectedCount=0;
            }
            else
                currentItemSelectedCount=1;//显示装备(在备选区中)或者卸下(在已经装备的武器区中)
            focus=StatusFocusOnItemSelected;
        ...
        case StatusMedicine:
            ...
            {
                if(currentEquipCount==MAXMEDICINECOUNT)
                {//装备区满了
                    if((*pArticleIDToInfoMap)[currentSelectedItemID].special>3000)
    ...
    case StatusFocusOnItemSelected:
        ...
        case StatusWeapon:
            ...
            {//在上面的框里,执行卸下操作
                ...
                for(int i=0;i<MAXWEAPONCOUNT;++i)
                {
                    if(roleInfo[currentRoleID-1]->weapon[i]==currentSelectedItemID)
                    {//定位到要卸下的物品处
                        memmove(&roleInfo[currentRoleID-1]->weapon[i],&roleInfo[currentRoleID-1]->weapon[i+1],sizeof(int)*(MAXWEAPONCOUNT-i-1));
                        //i=0移动3个字节,i=1移动2个字节
                ...
                if(pCarriedArticleSet->count(currentSelectedItemID)==1)
                {//证明刚刚卸下的东西原来不存在于装备区,因此需要刷新可用装备区,否则不需要做任何改变
                    this->resetAvailableArticle();
                    availableArticleBeginIter=availableArticleSet.begin();
                }
                currentSelectedItemID=roleInfo[currentRoleID-1]->weapon[currentItemIndex-1];
            }
            else
            {//执行装备操作,合法性由其他函数保证,即,如果装备区已满,那么此代码不应得到执行
                currentEquipCount++;
                for(int i=0;i<MAXWEAPONCOUNT;++i)
                {
                    if(roleInfo[currentRoleID-1]->weapon[i]==0)
                    {//定位到第一个为0的位置
                        roleInfo[currentRoleID-1]->weapon[i]=currentSelectedItemID;
                        break;
                    }
                }
                multiset<int>::iterator iter=pCarriedArticleSet->find(currentSelectedItemID);
                pCarriedArticleSet->erase(iter);
                if(pCarriedArticleSet->count(currentSelectedItemID)==0)
                {//刚刚装备的物品是可用装备区同等物品中的最后一件,因此需要刷新可用装备区,否则不需要任何改变
                    this->resetAvailableArticle();
                    availableArticleBeginIter=availableArticleSet.begin();
        ...
        case StatusShield:
            ...
            {//在上面的框里,执行卸下操作
                ...
                roleInfo[currentRoleID-1]->physicalDefence-=(*pArticleIDToInfoMap)[currentSelectedItemID].attack;
                //因为卸下了防具,所以防御降低
                //防具的特殊效果在这里进行
                pCarriedArticleSet->insert(currentSelectedItemID);
                currentItemIndex=0;
            ...
            {//执行装备操作,合法性由其他函数保证,即,如果装备区已满,那么此代码不应得到执行
                ...
                //找到并删除之
                if(pCarriedArticleSet->count(currentSelectedItemID)==0)
                {//刚刚装备的物品是可用装备区的最后一件,因此需要刷新可用装备区,否则不需要任何改变
                    this->resetAvailableArticle();
                    availableArticleBeginIter=availableArticleSet.begin();
                    if(!availableArticleSet.empty())
                    {
        ...
        case StatusArticle:
            ...
            {//在上面的框里,执行卸下操作
                ...
                if(special>ARTICLEDODGEUP_1-1
                        &&special<ARTICLEDODGEUP_10+1)
                {//将增加的躲闪减掉
                    roleInfo[currentRoleID-1]->dodge/=(1.0+(special-(ARTICLEDODGEUP_1-1))/10);
                }
                if(special>ARTICLEIQUP_1-1
                        &&special<ARTICLEIQUP_10+1)
                {//将增加的iq减掉
                    roleInfo[currentRoleID-1]->IQ/=(1.0+(special-(ARTICLEIQUP_1-1))/10);
                }
                if(special>ARTICLESPEEDUP_1-1
                        &&special<ARTICLESPEEDUP_10+1)
                {//将增加的移动减掉
                    roleInfo[currentRoleID-1]->speed-=(special-(ARTICLESPEEDUP_1-1));
                }
        ...
        case StatusMedicine:
            if(currentItemIndex<currentEquipCount+1)
            {//卸下或者使用某种药品,当前只允许使用回复HP类药品
                currentEquipCount--;
                //使用药品的合法性由其他函数保证,即:如果使用该药品,证明"使用"按钮是可用的
                if(currentItemSelectedIndex==1)
                {//使用的话加HP
                    int special=(*pArticleIDToInfoMap)[currentSelectedItemID].special-MEDICINEORIGIN;
                    assert(special>0&&special<1000);
                    roleInfo[currentRoleID-1]->HP+=special;
                    if((roleInfo[currentRoleID-1]->HP)>(roleInfo[currentRoleID-1]->HPMax))
                        roleInfo[currentRoleID-1]->HP=roleInfo[currentRoleID-1]->HPMax;
                }
                else
                {
                    pCarriedArticleSet->insert(currentSelectedItemID);
                ...
                for(int i=0;i<MAXWEAPONCOUNT;++i)
                {
                    if(roleInfo[currentRoleID-1]->medicine[i]==currentSelectedItemID)
                    {//定位到要卸下的物品处
                        memmove(&roleInfo[currentRoleID-1]->medicine[i],&roleInfo[currentRoleID-1]->medicine[i+1],sizeof(int)*(MAXMEDICINECOUNT-i-1));
                        //i=0移动3个字节,i=1移动2个字节
                ...
                else if(currentItemIndex==currentEquipCount+1)
                {//如果当前选中的是已经装备区的最后一条
                    currentItemIndex=1;
                    currentSelectedItemID=roleInfo[currentRoleID-1]->medicine[0];
                }
                else
                    currentSelectedItemID=roleInfo[currentRoleID-1]->medicine[currentItemIndex-1];
            ...
            {//装备或者使用某种药品
                ...
                else if((currentItemSelectedCount==1&&currentEquipCount==MAXMEDICINECOUNT)||
                        (currentItemSelectedCount==2&&currentItemSelectedIndex!=0))
                {//使用的话加HP
                    int special=(*pArticleIDToInfoMap)[currentSelectedItemID].special-MEDICINEORIGIN;
                    assert(special>0&&special<1000);
                    roleInfo[currentRoleID-1]->HP+=special;
                ...
                {
                    currentEquipCount++;
                    assert(roleInfo[currentRoleID-1]->medicine[MAXMEDICINECOUNT-1]==0);
                    roleInfo[currentRoleID-1]->medicine[currentEquipCount-1]=currentSelectedItemID;
                }
...
void StatusWidget::onDirectionKey(DIRECTION dir)
{
    ...
    case DOWN:
        ...
        case StatusFocusOnEquipment:
            ...
            else
            {
                ...
                if(currentAvailableCount>AVAILABLEARTICLECOUNT)
                {
                    if(currentItemIndex==currentEquipCount+AVAILABLEARTICLECOUNT)//到达了可用栏的最后一条
                    {
                        int i=AVAILABLEARTICLECOUNT;
                        set<int>::iterator iter=availableArticleBeginIter;
                        while(i--)
    ...
    case LEFT:
        ...
        case StatusFocusOnTabIndex:
            if(currentTabIndex==StatusRoleInfo)
            {//光标在人物ID栏,按左箭头切换到最右index
                currentTabIndex=StatusArticle;
            }
            else
            {
                currentTabIndex=static_cast<StatusTabIndex>(static_cast<int>(currentTabIndex)-1);
            }
            currentItemIndex=0;
            currentItemSelectedIndex=0;
            if(currentTabIndex!=StatusRoleInfo)
    ...
    case RIGHT:
        ...
        {
        case StatusFocusOnTabIndex:
            if(currentTabIndex==StatusArticle)
            {//光标在人物ID栏,按左箭头切换到最右index
                currentTabIndex=StatusRoleInfo;
            }
            else
...
void StatusWidget::resetAvailableArticle()
{
    ...
    case StatusWeapon:
        ...
        for(int i=0;i<MAXWEAPONCOUNT;++i)
        {
            ...
            {
                if(pArticleIDToInfoMap->find(roleInfo[currentRoleID-1]->weapon[i])==pArticleIDToInfoMap->end())
                {//没找到
                   DBIO dbio;
                   if(!dbio.getArticleInfoFromLib(roleInfo[currentRoleID-1]->weapon[i],pArticleIDToInfoMap))
        ...
        {
            int currentRoleJob=(static_cast<int>(roleInfo[currentRoleID-1]->job))/3;
            if((*iter)/100==currentRoleJob)
            {
                if(pArticleIDToInfoMap->find(*iter)==pArticleIDToInfoMap->end())
                {//没找到
                   DBIO dbio;
    ...
    case StatusShield:
        ...
        {
            if(pArticleIDToInfoMap->find(roleInfo[currentRoleID-1]->shield)==pArticleIDToInfoMap->end())
            {//没找到
               DBIO dbio;
               if(!dbio.getArticleInfoFromLib(roleInfo[currentRoleID-1]->shield,pArticleIDToInfoMap))
                   quitApp(ERRORGETARTICLEINFOFAIL);
            }
           currentEquipCount++;
        }
    ...
    case StatusMedicine:
        ...
        for(int i=0;i<MAXMEDICINECOUNT;++i)
        {
            ...
            {
                if(pArticleIDToInfoMap->find(roleInfo[currentRoleID-1]->medicine[i])==pArticleIDToInfoMap->end())
                {//没找到
                   DBIO dbio;
                   if(!dbio.getArticleInfoFromLib(roleInfo[currentRoleID-1]->medicine[i],pArticleIDToInfoMap))
                       quitApp(ERRORGETARTICLEINFOFAIL);
                }
                currentEquipCount++;
            }
    ...
    case StatusArticle:
        ...
        {
            if(pArticleIDToInfoMap->find(roleInfo[currentRoleID-1]->article)==pArticleIDToInfoMap->end())
            {
                DBIO dbio;
                if(!dbio.getArticleInfoFromLib(roleInfo[currentRoleID-1]->article,pArticleIDToInfoMap))
                    quitApp(ERRORGETARTICLEINFOFAIL);
            }
            currentEquipCount++;
        }
...
//    for(set<int>::iterator iter=availableArticleSet.begin();iter!=availableArticleSet.end();++iter)
//    {
//        qDebug()<<(*iter);
//    }
    this->update();
