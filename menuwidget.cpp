...
MenuWidget::MenuWidget(QWidget *parent, KeyboardControl *key, StoryMenuOrBattleMenu which) :
    QWidget(parent),
    currentIndex(0),
    key(key)
{
...
void MenuWidget::paintEvent(QPaintEvent *)
{
    ...
    if(indexNum==4)
    {
        ...
        painter.drawText((WINDOWWIDTH-MENUWIDTH)/2,(WINDOWHEIGHT-MENUHEIGHT)/2,MENUWIDTH,MENUHEIGHT,Qt::AlignCenter,QString("状态\n存档\n读档\n回主菜单"));
        painter.setPen(QColor(Qt::white));
        painter.setBrush(Qt::NoBrush);
        painter.drawRoundedRect((WINDOWWIDTH-MENUWIDTH)/2+5,
                                (WINDOWHEIGHT-MENUHEIGHT)/2+currentIndex*(MENUHEIGHT/4-5)+10,
                                MENUWIDTH-10,
                                MENUHEIGHT/4-6,3,3);
    }
    else
    {
        ...
        painter.setPen(QPen(QColor(241,252,254)));
        painter.setFont(font);
        painter.drawText((WINDOWWIDTH-MENUWIDTH)/2,(WINDOWHEIGHT-MENUHEIGHT)/2,MENUWIDTH,MENUHEIGHT,Qt::AlignCenter,QString("回合结束\n存档\n读档\n战斗目的\n打不过跑"));
        painter.setPen(QColor(Qt::white));
        painter.setBrush(Qt::NoBrush);
        painter.drawRoundedRect((WINDOWWIDTH-MENUWIDTH)/2+5,
                                (WINDOWHEIGHT-MENUHEIGHT)/2+currentIndex*(MENUHEIGHT/4-8)+5,
                                MENUWIDTH-10,
                                MENUHEIGHT/4-6,3,3);
    }
...
void MenuWidget::keyReleaseEvent(QKeyEvent *event)
{
    ...
    {
        if(currentIndex==0)currentIndex=indexNum-1;
        else currentIndex--;
    }
    else if(event->key()==key->downKey)
    {
        if(currentIndex==indexNum-1)currentIndex=0;
        else currentIndex++;
    }
