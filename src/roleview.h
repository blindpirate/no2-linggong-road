...
struct RoleViewData {
  RoleViewData(int pointMapX, int pointMapY, int ID, int currentIndexInPathVec,
               int type, ROLEPOSE currentPose, DIRECTION dir, bool isActive,
               bool reactOnViewMove, bool contactDirectly,
               bool animationTimerOn, bool moveTimerOn)
      : pointX(pointMapX), pointY(pointMapY), ID(ID),
        currentIndexInPathVec(currentIndexInPathVec), type(type),
        currentPose(currentPose), dir(dir), isActive(isActive),
        reactOnViewMove(reactOnViewMove), contactDirectly(contactDirectly),
        animationTimerOn(animationTimerOn), moveTimerOn(moveTimerOn) {}
  int pointX; //左上角点坐标
  int pointY;
  int ID;
...
public:
  ...
  // void sweat(bool
  // endWithSignal);//流汗,true为完成后发射信号,false为不发送,这样就可以实现多动画同时进行而只需要发射一个信号
  // void attack();//只是展示动画效果
  // void recover(int originalHP,int deltaHP,int fullHP,bool
  // endWithSignal);//原始体力,回复体力,满体力 void hurt(int originalHP,int
  // deltaHP,int fullHP,bool endWithSignal);//原始体力,扣减体力,满体力 void
  // die();
...
public slots:
  ...
  void onViewMovedTo(int x,
                     int y); //若视角移动到x,y像素点处,需要自身作出一定的调整
...
private:
  ...
  int **mapArray;
  vector<QPoint> *
      pPathVec; //移动路径点vector,循环显示,到达最后一个点之后重新从第一个点开始移动.如果不需要循环移动,则最后一个点坐标是(-1,-1)
  int currentIndexInPathVec;
  int pointMapX;
  int pointMapY;
  int mapX; //在地图中的坐标
  ...
  // bool isMoving;
  bool
      contactDirectly; //这个标志位为真的话,只要移动的下一格是该view,就会触发onContact()函数,否则需要按action键才能触发onContact函数
private:
  void changePose();
...
{//在storyscene中的主角和battlescene中的所有角色都是Hero类的
    Q_OBJECT
public:
    explicit Hero(QWidget*parent,int mapX,int mapY,QString &,QString
&);//两个参数分别是param1和param2 ~Hero(); void setReactOnViewMove(bool
value){reactOnViewMove=value;} Role *getRole(){return role;} void
attack();//只是展示动画效果 void
recovery(int,int,int);//原始体力,回复体力,满体力 void
hurt(int,int,int);//原始体力,扣减体力,满体力 void die(); void dodge();//闪避
    bool getActiveStatus(){return isActive;}
