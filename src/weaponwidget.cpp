...
WeaponWidget::WeaponWidget(QWidget *parent,
    ...
    : QWidget(parent), pArticleIDToInfoMap(pArticleIDToInfoMap),
      type(ShowNoWeaponNorMedicine), x(0), y(0), role(NULL), currentIndex(0),
      weaponSum(0) {
  this->hide();
  ...
  if (type == ShowMedicine) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      painter.drawText(x, y + (i + 1) * 20, 120, 20, Qt::AlignCenter,
                       (*pArticleIDToInfoMap)[role->medicine[i]].name);
    }
    if (weaponSum) {
      painter.setPen(QPen(QBrush(QColor(Qt::red)), 2));
      painter.drawText(
          x, y + (currentIndex + 1) * 20, 120, 20, Qt::AlignCenter,
          (*pArticleIDToInfoMap)[role->medicine[currentIndex]].name);
    }
    ...
    for (int i = 0; i < 4; ++i) {
      if (!(role->weapon[i]))
        continue;
      painter.drawText(
          x, y + (i + 1) * 20, 120, 20, Qt::AlignCenter,
          QString("%1 %2 %3")
...
void WeaponWidget::show(int rolePointX, int rolePointY, Role *roleInfo,
                        WeaponOrMedicine newType) {
  assert(roleInfo);
  x = rolePointX;
  y = rolePointY + 36;
  role = roleInfo;
  ...
  if (type == ShowMedicine) {
    for (int i = 0; i < 4; ++i) {
      ...
      weaponSum++;
      if (pArticleIDToInfoMap->find(role->medicine[i]) ==
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      weaponSum++;
      if (pArticleIDToInfoMap->find(role->weapon[i]) ==
          pArticleIDToInfoMap->end()) {
        DBIO dbio;
