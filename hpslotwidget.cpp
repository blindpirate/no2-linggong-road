...
HPSlotWidget::HPSlotWidget(QWidget *parent,int rolePointX,int rolePointY,int startHP, int deltaHP, int fullHP,AnimationOperation op):
    QWidget(parent),
    startHP(startHP),
    deltaHP(deltaHP),
    fullHP(fullHP),
...
//    }
    ...
    if(op==AniRoleHurt)
    {
        QPropertyAnimation *animation=new QPropertyAnimation(this,"currentHP",this);
        ...
        QPropertyAnimation *rectAnimation=new QPropertyAnimation(this,"numRect",this);
        ...
        connect(rectAnimation,SIGNAL(finished()),rectAnimation,SLOT(deleteLater()));

        connect(rectAnimation,SIGNAL(finished()),this,SLOT(close()));
        if(endWithSignal)
            connect(rectAnimation,SIGNAL(finished()),this->parent(),SLOT(startAnimation()));
...
void HPSlotWidget::paintEvent(QPaintEvent *)
{
    ...
    if(op==AniRoleHurt)
    {
        ...
        painter.setFont(font);
        painter.drawText(this->numRect,Qt::AlignCenter,QString("%1").arg(deltaHP));
    }
